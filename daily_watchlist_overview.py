...
SCANNER_VERSION = "2026.08.30-balanced-v1.7"
LEARNING_MODEL_VERSION = "five-session-execution-v10"
INCREMENTAL_STATE_VERSION = "incremental-state-v3"
...
def supabase_upsert(table: str, records: list[dict], conflict_columns: list[str]) -> None:
    ...
    for attempt in range(1, max(1, SUPABASE_UPSERT_MAX_ATTEMPTS) + 1):
        ...
        try:
            ...
            if not retryable or attempt >= SUPABASE_UPSERT_MAX_ATTEMPTS:
                raise RuntimeError(f"Supabase upsert to {table} failed with HTTP {exc.code}: {body}") from exc
        except (TimeoutError, URLError, OSError) as exc:
            if attempt >= SUPABASE_UPSERT_MAX_ATTEMPTS:
                raise RuntimeError(
                    f"Supabase upsert to {table} failed after {attempt} attempts: {exc}"
                ) from exc
        delay = SUPABASE_UPSERT_RETRY_BASE_SECONDS * (2 ** (attempt - 1))
        print(f"Supabase upsert to {table} transiently failed; retrying batch in {delay:.2f}s.")
        time.sleep(delay)
...
SUPABASE_STORAGE_OVERHEAD_FACTOR = 2.0
...
def score_signal_horizon(prior: dict, future_rows: list[dict], horizon_sessions: int = LEARNING_HORIZON_SESSIONS) -> dict:
    ...
    for offset, bar in enumerate(future_rows[:horizon], start=1):
        ...
        if any(value is None for value in (open_, high, low, close)):
            ...
            return base
        # Calibration is a frozen-plan OHLC experiment. Replayed scanner
        # actions and operator states are regenerated by today's code and can
        # drift from what production knew on that future date. Feeding those
        # mutable classifications back into settlement makes a clean rebuild
        # change an otherwise identical historical outcome. The live plan
        # lifecycle still honors hard-risk states; learning settlement uses
        # only the canonical market bar.
        market_bar = {
            key: bar.get(key)
            for key in ("date", "history_date", "data_date", "open", "high", "low", "close")
        }
        plan = evaluate_execution_plan(plan, market_bar)
